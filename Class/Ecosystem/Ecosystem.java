...
/**
 * An ecosystem simulation. Part 1.
 ...
 * Animal object, which can be a Bear object, a Fish object, or null. In each
 * time step, based on a random process, each animal either attempts to move
 * into an adjacent array cell or stay where it is. If two animals of the same
 * type are about to collide in the same cell, then they stay where they are,
 * but they create a new instance of that type of animal, which is placed in a
 * random empty (i.e., previously null) cell in the array. If a bear and a fish
 * collide, however, then the fish dies (i.e., it disappears). Use actual object
 * creation, via the new operator, to model the creation of new objects, and
...
public class Ecosystem extends CE {
    ...
    public Ecosystem(String[] args) {
        print("\n~~~~~~~~~~~~\nEcosystem Simulation Part 1. By Alexander Irausquin-Petit (2023)\n~~~~~~~~~~~~\n");
    ...
    private void createNewAnimal(Animal parent) {
        ...
        if (randomTile == null) {
            if (!quietMode)
                print("Could not find a random tile. Is the ecosystem full? (this warning is hidden in Stealthy Operation Mode)");
    ...
    private void printTileset(boolean force) {
        ...
        for (int row = 0; row < SIZE; row++) {
            for (int col = 0; col < SIZE; col++) {
                ...
                fishPopulation += tileString.toLowerCase().equals("f") ? 1 : 0;
                bearPopulation += tileString.toLowerCase().equals("b") ? 1 : 0;
            }
