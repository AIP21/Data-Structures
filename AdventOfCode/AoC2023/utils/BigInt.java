...
public class BigInt extends Number implements Comparable<BigInt> {
    /**
     * Used to cast a (base 2^32) digit to a long without getting unwanted sign
     * extension.
     */
    ...
    /**
     * Assigns the given BigInt parameter to this number.
     * Assumes no leading zeroes of the input-array, i.e. that v[vlen-1]!=0, except
     ...
     *         obviously being implicit),
     *         the exponent value which will be consistent for {@code BigInt}s up to
     *         128 bits (should it not fit it'll be calculated modulo 256),
     ...
     *         obviously being implicit),
     *         the exponent value which will be consistent for {@code BigInt}s up to
     *         1024 bits (should it not fit it'll be calculated modulo 2048),
     *         and the sign bit set if this number is negative.
     */
    @Override
    ...
    private void safeUrem(final int mod) {
        ...
        for (int i = len - 1; i >= 0; i--) {
            ...
            // Because hrem < 2^32 and there's at least one zero bit in [62,32] if bit 63 is
            // set.
        }
        len = 1;
        dig[0] = (int) rem;
     ...
     * @complexity O(n)
     */
    public void add(final int add) // Has not amortized O(1) due to the risk of alternating +1 -1 on continuous
                                   // sequence of 1-set bits.
    {
        if (add < 0)
            usub(-add);
        else
     ...
     *
     * @param mul      The number to multiply with.
     * @param parallel true if we should try to parallelize the algorithm, false if
     *                 we shouldn't.
     * @complexity O(n^1.585)
     */
     ...
     * differs from {@code rem} in that it always computes * <i>non-negative</i>
     * result
     *
     * @param div The number to use in the division causing the remainder.
     * @see #rem
    ...
    // sign extension.
    // This has no effect on bit shifts, but makes implementaion of other bit
    // operations a bit
    ...
    /**
     * Shifts this number left by 32*shift, i.e. moves each digit shift positions to
     * the left.
     *
    ...
    private void bigShiftRight(final int shift) {
        ...
        // for(int i = len-shift; i<len; i++) dig[i] = 0; dig[j >= len] are allowed to
        // be anything.
        len -= shift;
    }
